...
export default async function KontakPage() {
  ...
  const csUrl = generateWhatsAppCSURL(settings.whatsappCSNumber);

  const contactCards = [
    {
      icon: WhatsAppIcon,
      label: "WhatsApp",
      value: formatPhone(settings.whatsappCSNumber),
      hint: "Respon cepat",
    },
    {
      icon: Mail,
      ...
      <Section className="py-14">
        <Container>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {contactCards.map((c) => (
              <div key={c.label} className="flex flex-col gap-3 rounded-lg border border-brand-border bg-white p-6">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-brand-cream text-brand-primary">
                  <c.icon className="h-5 w-5" />
                </div>
                <div>
                  <p className="text-xs uppercase tracking-wider text-brand-text-muted">{c.label}</p>
      ...
      <Section className="bg-brand-cream/60 border-y border-brand-border">
        <Container>
          ...
          <div className="max-w-3xl mx-auto">
            <FAQ />
            <div className="mt-10 rounded-xl bg-brand-primary text-white p-6 flex flex-col sm:flex-row sm:items-center gap-5 justify-between">
              <div className="flex items-start gap-3">
                <MessageCircle className="h-6 w-6 text-brand-secondary shrink-0 mt-1" />
                <div>
